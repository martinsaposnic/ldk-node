...
use lightning::ln::channelmanager::{Bolt11InvoiceParameters, PaymentId, Retry};
...
impl Bolt11Payment {
	...
	pub fn send(
		...
		match self.channel_manager.pay_for_bolt11_invoice(
			...
			Err(_) => {
				println!("Failed to send payment due to the given invoice being invalid or incompatible.");
	...
	pub fn receive_via_jit_channel_manual_claim(
		...
		let (invoice, preimage) = self.receive_via_jit_channel_inner(
			...
			max_total_lsp_fee_limit_msat,
			None,
			false, // auto_claim = false
		)?;
		let preimage = preimage.ok_or(Error::InvoiceCreationFailed)?;
		Ok((maybe_wrap_invoice(invoice), preimage))
	}
